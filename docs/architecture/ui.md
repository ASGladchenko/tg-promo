...
- Не создавай длинные CSS-классы, которые перечисляют весь путь компонента или несколько уровней
  ответственности. Держи локальные имена короткими: `block__element`, `block--modifier`; не делай
  имена вроде `ai-provider-api-key-modal-trigger__button`.
