...
export default function LotteryCodePicker({
  digits,
  activeIndex,
  ...
  useLayoutEffect(() => {
    ...
    digits.forEach((digit, columnIndex) => {
      ...
      const currentIndex = clamp(Math.round(column.scrollTop / getColumnStep(column)), 0, visibleDigits.length - 1);
  ...
  function handleScroll(columnIndex: number) {
    ...
    const visibleDigits = getAvailableDigits(columnIndex, digits, hideSelectedDigitsFromOtherColumns);
    const digitIndex = clamp(Math.round(column.scrollTop / getColumnStep(column)), 0, visibleDigits.length - 1);
    ...
    <div className="lottery-code-picker">
      <div className="lottery-code-picker__viewport" aria-label="Выберите три цифры кода">
        {Array.from({ length: 3 }, (_, columnIndex) => {
          const visibleDigits = getAvailableDigits(columnIndex, digits, hideSelectedDigitsFromOtherColumns);
          const selectedDigit = visibleDigits.includes(digits[columnIndex]) ? digits[columnIndex] : visibleDigits[0];

          return (
            <div
              className={[
                "lottery-code-picker__column",
                activeIndex === columnIndex ? "lottery-code-picker__column--active" : "",
              ]
                .filter(Boolean)
                .join(" ")}
              key={columnIndex}
            ...
            >
              ...
              {visibleDigits.map((digit, digitIndex) => (
                <button
                  className={[
                    "lottery-code-picker__digit",
                    selectedDigit === digit ? "lottery-code-picker__digit--selected" : "",
                  ]
                    .filter(Boolean)
                    .join(" ")}
                  key={digit}
